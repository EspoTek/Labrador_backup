...
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_unixUsbDriver_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
...
const QMetaObject unixUsbDriver::staticMetaObject = {
    { &genericUsbDriver::staticMetaObject, qt_meta_stringdata_unixUsbDriver.data,
      qt_meta_data_unixUsbDriver,  qt_static_metacall, 0, 0}
};
...
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_unixUsbDriver.stringdata))
